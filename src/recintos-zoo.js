class RecintosZoo {
  analisaRecintos(animal, quantidade) {
    ...
    recintos.forEach((recinto) => {
      let biomaCompativel =
        biomasAceitaveis.some((bioma) => recinto.bioma.includes(bioma)) ||
      ...
      if (
        carnivoro &&
        especiesNoRecinto.length > 0 &&
        !especiesNoRecinto.includes(animal)
      ) {
        return;
      }

      // Verificar se já há carnívoros no recinto (não pode misturar macacos com carnívoros)
      const existeCarnivoro = especiesNoRecinto.some(
        (especie) => animais[especie].carnivoro
      );
